// Poller de vendas do LEANDRO — RedTrack -> ntfy (roda no GitHub Actions)
// Filtro: campanha que NAO contem "TORRES" E payout > 0 (as TORRES sao do Klisman)
// Estado proprio em state/seen-leandro.json — independente do poller TORRES.
import fs from "fs";

const API_KEY = process.env.REDTRACK_API_KEY;
const NTFY_TOPIC = process.env.NTFY_TOPIC_LEANDRO;
const STATE = "state/seen-leandro.json";
if (!API_KEY) { console.error("REDTRACK_API_KEY ausente"); process.exit(1); }
if (!NTFY_TOPIC) { console.log("NTFY_TOPIC_LEANDRO ausente — poller leandro desativado"); process.exit(0); }
...
const vendas = all.filter(
  (c) => !/torres/i.test(c.campaign || "") && (c.type || "") === "Purchase" && Number(c.payout) > 0
);
